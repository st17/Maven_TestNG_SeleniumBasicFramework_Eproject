...
public class TC_108 {
    ...
    //Verify that navigate to "listCampaignType" page when user clicks on "Cancel" button
    @Test
    public void cancel_CampaignType() throws IOException {
        ...
        Campaign_Action.clickCancel(dr);
        if (dr.findElement(By.xpath("//strong[text()='Show All Campaigns Type']")).isDisplayed() == true) {
            ex1.setTC_ID("76");
            ex1.setTC_Summary("Verify that navigate to \"listCampaignType\" page when user clicks on \"Cancel\" button");
            ex1.setTC_Result("Passed");
            ex1.setTC_Note("TC_108");
        } else {
            ex1.setTC_ID("76");
            ex1.setTC_Summary("Verify that navigate to \"listCampaignType\" page when user clicks on \"Cancel\" button");
