...
public class TR015_TC72 {
    ...
    //Verify that "Interested product", "Date to make opportunity", "Quantity" fields display when user clicks on "Add Opportunity" button
    @Test
    public void check_fieldOpportunity() throws IOException {
        ...
        if (dr.findElement(By.xpath("//th[text()='Check']")).isDisplayed() == true && (dr.findElement(By.xpath("//th[text()='Name']"))).isDisplayed()==true
                && (dr.findElement(By.xpath("//th[text()='Price']"))).isDisplayed()==true) {
            ex1.setTC_ID("80");
            ex1.setTC_Summary("Verify that \"Interested product\", \"Date to make opportunity\", \"Quantity\" fields display when user clicks on \"Add Opportunity\" button");
            ex1.setTC_Result("Failed");
            ex1.setTC_Note("TC_72");
        } else {
            ex1.setTC_ID("80");
            ex1.setTC_Summary("Verify that \"Interested product\", \"Date to make opportunity\", \"Quantity\" fields display when user clicks on \"Add Opportunity\" button");
